...
class PhotometryStatus:
    ...
    def show_status(self):
        try:
            ...
            log('observing_run must be O4a, O4b, or O4c')
            return
        #TODO: automate
        ...
        log(f'{len(events_dict) - len(good_events)} / {len(events_dict)} events in {self.observing_run} are not priority')
...
class PhotometryCoords():
    ...
    def get_agn_coords(self):
        ...
        if self.action == 'all':
            log(f'about {len(ztf_coords)} / {len(all_coords)} coords should be in ZTF footprint')
        ...
        if self.action == 'new':
            ...
            log(f'{len(new_coords)} / {len(all_coords)} total coords dont have photometry')
        ...
        if self.action == 'update':
            if events_dict[self.graceid]['flare']:
                ...
                log(f'last photometry request for {self.graceid} was on {date_zfps}')
            else:
                log(f'Error: no photometry for {self.graceid}')  
                return
            existing_coords = [coords for coords,name in zip(ztf_coords, names) if os.path.exists(path + name + '.gz')]
            log(f'Found saved photometry for {len(existing_coords)} / {len(all_coords)} coords crossmatched')
            ...
            if num_invalid > 0:
                log(f'{num_invalid} coords have photometry within a week or are outside 200 days post GW')
            elif len(filtered_coords) == 0:
                log(f'No coords valid for update photometry')
            else:
                log(f'{len(filtered_coords)} coords are valid for update photometry')
            #convert mjd to jd
    ...
    def custom_update_batching(self, coords, dates, threshold=60):
        ...
        log(f'After batching dates with window size {threshold}, created {len(single_dates)} batches')
    ...
    def format_for_zfps(self, coords, date):
        ...
        if len(coords) == 0:   
            ...
            log(f'Submit in {len(ralist)} batches')
    ...
    def replace_scientific_notation(self, ra_list, dec_list):
        ...
        def process_list(lst):
            if all(isinstance(i, list) for i in lst):
                ...
                if len(sci) > 0:
                    log(f'Found sci: {sci}')
                    log('If there is a number in scientific notation that shouldn\'t be rounded to zero, need to address manually')
    ...
    def get_photometry_coords(self):
        ...
        if self.action == 'update':
            ...
            log(f'After batching for ZFPS, retrieved {len(date)} objects in {len(jd)} batches')
        else:
        ...
        if num_agn > 15000:
            ...
            log(f'Retrieved {num_agn} AGN for submission now')
    ...
    def queue_photometry(self, ra, dec, jd, number_to_submit):
        ...
        log(f"Saving {len(ra)} queued photometry to {file_path}")
    ...
    def move_complete_queued_photometry(file_name, path_data):
        ...
        if os.path.exists(source_path):
            ...
            log(f"Moved {file_name} to {path_complete_queued_photometry}")
        else:
            log(f"{file_name} not found in {path_queued_photometry}")
...
class GetPhotometry():
    ...
    def submit(self):
        if len(self.ra) == 0:
            ...
            if self.testing:
                log('Testing mode - no submission')
            else:
                log(f'submit in {num_batches} batches')
            ...
            if self.testing:
                ...
                log('submit in one batch')  
                self.submit_post(self.ra, self.dec, self.jd)         
        photometry_date = self.save_photometry_date()
        log(f'Submitted {num_agn} AGN in {num_batches} batches at {photometry_date}')
...
class SavePhotometry():
    ...
    def get_coords_batchcode(self):
        ...
        if r.status_code == 200:
            log("Script executed normally and queried the ZTF Batch Forced Photometry database.\n")
            ...
            log(f'{len(table_gw)} coords found')
    ...
    def get_coords_graceid(self):
        ...
        log(f'{len(filtered_table)} coords found')
        ...
        log(f'Returned {num_batches_received} batches for {self.num_batches_submitted} submitted')
    ...
    def get_photometry(self):
        ...
        if r.status_code == 200:
            ...
            if lightcurves is not None:
                ...
                log("Status_code=",r.status_code,"; Jobs either queued or abnormal execution.")
        batch_lightcurves = [lc for lc in batch_url if any(batch in lc for batch in self.batch_codes)]
        log("Retrieved", len(batch_lightcurves), "lightcurves")
    ...
    def save(self): 
        ...
        log(f'{num_errors} broken urls; {len(values)} lightcurves returned')
        ...
        if self.action == 'update':
            ...
            log(f'loaded {len(batch_photometry_existing)} existing AGN photometry for {len(lc_cut)} new AGN photometry')
            ...
            if len(batch_photometry_existing) == len(filename_updated):
                ...
                log('Error: different number of lightcurves and filenames')
                return None   
        ...
        log(f'downloaded {num_returned} lightcurves')
...
class PhotometryLog():
    ...
    def check_completed_events(self):
        ...
        for id in self.photometry_pipeline['events'].keys():
            ...
            if (Time.now() - Time(dateobs, format='isot')).value > 200:
                ...
                log(f'Event {id} is over 200 days old')
    ...
    def check_num_pending_zfps(self):
        ...
        if r.status_code == 200:
            ...
            log(f"Number of pending requests: {num_pending}")
            return num_pending
        else:
    ...
    def add_event(self, event_id, event_data):
        ...
        if event_id not in self.photometry_pipeline['events']:
            ...
            with open(self.path_pipeline, 'w') as file:
                json.dump(self.photometry_pipeline, file, indent=4)
            log(f"Event ID {event_id} added successfully.")
        else:
            log(f"Event ID {event_id} already exists in the photometry pipeline.")
    ...
    def add_zfps_entry(self, event_id, new_entry):
        ...
        if event_id in self.photometry_pipeline['events']:
            ...
            with open(self.path_pipeline, 'w') as file:
                json.dump(self.photometry_pipeline, file, indent=4)
            log(f"New ZFPS added to {event_id}.")
        else:
            log(f"Event ID {event_id} not found in the photometry pipeline.")
    ...
    def check_photometry_status(self):
        ...
        log(f'Waiting for photometry: {[x[0] for x in waiting_for_photometry]}')
        log(f'Needs photometry request: {[x[0] for x in needs_photometry_request]}')
        ...
        if event_id in self.photometry_pipeline['events']:
            for x in self.photometry_pipeline['events'][event_id]['zfps']:
                if x['submission_date'] == submission_date:
                    ...
                    with open(self.path_pipeline, 'w') as file:
                        json.dump(self.photometry_pipeline, file, indent=4)
                    log(f"Photometry status updated for {event_id}.")
                    return
            log(f"Submission date {submission_date} not found for {event_id}.")
        else:
            log(f"Event ID {event_id} not found in the photometry pipeline.")
...
class PlotPhotometry():
    ...
    def plot_photometry_dates(self):
        ...
        log(f'{len(empty)} / {len(batch_photometry_filtered)} dataframes for {total_matches} Catnorth sources are empty')
