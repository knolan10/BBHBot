...
class PhotometryStatus:
    ...
    def show_status(self):
        ...
        zfps_status_df = pd.DataFrame(
            {
                ...
                "Date last zfps": [
                    value["flare"]["date_last_zfps"]
                    if "date_last_zfps" in value["flare"]
                    else ""
                    for value in good_events_dict.values()
                ],
                "Status": "",
                "Trigger": [
                    True if key in trigger_list else ""
                    for key in good_events_dict.keys()
                ],
            }
        )
        ...
        no_crossmatch_agn = [
            key
            for key, value in good_events_dict.items()
            if value["crossmatch"]["n_agn_catnorth"] == 0
        ]
        # assign event status
        ...
        def assign_status(row):
            if row["ID"] in no_crossmatch_agn or row["Date last zfps"] == "NA":
                ...
                try:
                    date_last_zfps = datetime.strptime(
                        row["Date last zfps"], "%Y-%m-%d %H:%M:%S.%f"
                    )
...
class PhotometryCoords:
    ...
    def get_agn_coords(self):
        ...
        if self.action == "update":
            ...
            latest_dates = [
                round(df["jd"].max()) if not df.empty else 2459367.5
                for df in existing_photometry
            ]
            current_date = Time.now().jd
            # dont request if photometry from within the week, or we already have 200 days post gw
            filtered_coords = [
                x
                for x, date in zip(existing_coords, latest_dates)
                if current_date - date > 7 and date - gw_jd < 200
            ]
    ...
    def custom_update_batching(self, coords, dates, threshold=60):
        ...
        for i in range(len(sorted_dates)):
            if (
                not current_group_dates
                or sorted_dates[i] - current_group_dates[-1] <= threshold
            ):
    ...
    def get_photometry_coords(self):
        ...
        if self.action == "update":
            ...
            formatted = [
                self.format_for_zfps(i, j)
                for i, j in zip(grouped_coords, grouped_dates)
            ]
    ...
    def queue_photometry(self, ra, dec, jd, number_to_submit):
        ...
        """
        file_path = (
            f"{self.path_data}/flare_data/queued_for_photometry/{self.graceid}.json"
        )
            ...
            if file_name.endswith(".json"):
                ...
                with open(file_path, "r") as file:
                    ...
                    photometry_data.append(
                        (id, ra, dec, jd, number_to_submit, action, file_name)
                    )
    ...
    def move_complete_queued_photometry(file_name, path_data):
        ...
        """
        path_queued_photometry = f"{path_data}/flare_data/queued_for_photometry"
        path_complete_queued_photometry = (
            f"{path_data}/flare_data/completed_queued_photometry"
        )
...
class SavePhotometry:
    ...
    def get_coords_graceid(self):
        ...
        full_table["ra_truncated"] = full_table["ra"].apply(
            lambda x: truncate_to_precision(x)
        )
        full_table["dec_truncated"] = full_table["dec"].apply(
            lambda x: truncate_to_precision(x)
        )
        crossmatch_df["ra_truncated"] = crossmatch_df["ra"].apply(
            lambda x: truncate_to_precision(x)
        )
        crossmatch_df["dec_truncated"] = crossmatch_df["dec"].apply(
            lambda x: truncate_to_precision(x)
        )
    ...
    def df_from_url(self, url, file):
        ...
        data = requests.get(
            ...
            data={"email": self.email, "userpass": self.userpass},
        )
        if data.status_code == 200:
            df = pd.read_csv(
                io.StringIO(data.content.decode("utf-8")), sep=r"\s+", comment="#"
            )
    ...
    def load_event_lightcurves(self, filename):
        ...
        df = [
            pd.read_pickle(dir + file + ".gz", compression="gzip")
            for file in filename
            if os.path.exists(dir + file + ".gz")
        ]
    ...
    def save(self):
        ...
        if (
            retrieved_photometry is None
        ):  # if not all batches are returned (could be partially returned)
            return None
        filename = retrieved_photometry[1]
        if self.batch_codes is None:
            self.batch_codes = retrieved_photometry[2]
        lightcurves = self.get_photometry()
        result = [
            self.df_from_url(url, file) for url, file in zip(lightcurves, filename)
        ]
            ...
            if len(batch_photometry_existing) == len(filename_updated):
                if self.testing:
                    ...
                    [
                        ...
                        for i, j in zip(batch_photometry_existing, filename_updated)
                    ]
        ...
        # save the new photometry
            ...
            if len(lc_cut) == len(filename_updated):
                if self.testing:
                    ...
                    [
                        self.download_lightcurves(i, j)
                        for i, j in zip(lc_cut, filename_updated)
                    ]
...
class PhotometryLog:
    ...
    def check_completed_events(self):
        ...
        for id in self.photometry_pipeline["events"].keys():
            if (
                self.photometry_pipeline["events"][id]["over_200_days"]
                or "dateobs" not in self.photometry_pipeline["events"][id]
            ):
                continue
    ...
    def update_summary_stats(self, number_requested, number_saved, keyword):
        ...
        if keyword == "new_request":
            self.photometry_pipeline["summary_stats"]["total_requests"] += (
                number_requested
            )
    ...
    def save_num_pending(self, num_pending):
        ...
        use pending value to prevent from having > 20,000 requests at once, which ZFPS wont allow
        """
        self.photometry_pipeline["summary_stats"]["total_currently_pending"] = (
            num_pending
        )
        with open(self.path_pipeline, "w") as file:
            json.dump(self.photometry_pipeline, file, indent=4)
    ...
    def check_photometry_status(self):
        ...
        for id in self.photometry_pipeline["events"].keys():
            ...
            dateobs = self.photometry_pipeline["events"][id]["dateobs"]
            time_delta = round(
                (Time.now() - Time(dateobs, format="isot")).to_value("jd")
            )
            ...
            for x in self.photometry_pipeline["events"][id]["zfps"]:
                if not isinstance(x, str) and not x["complete"]:
                    waiting_for_photometry.append(
                        [
                            id,
                            x["submission_date"],
                            x["num_batches_submitted"],
                            x["action"],
                        ]
                    )
            # find events that need update photometry request based on our cadence (loosely based on followup TOO schedule)
            num_zfps_requests_so_far = len(
                [
                    x
                    for x in self.photometry_pipeline["events"][id]["zfps"]
            ...
            num_requests_should_be_made = [
                ...
                6,
                7,
            ]  # ie after 9 days, we should have 2 requests (the initial and a first update)
            if any(
                time_delta >= t and num_zfps_requests_so_far < z
    ...
    def update_photometry_complete(
        self, event_id, submission_date, batch_ids, num_returned, num_broken
    ):
        """
        ...
        if event_id in self.photometry_pipeline["events"]:
            for x in self.photometry_pipeline["events"][event_id]["zfps"]:
                if x["submission_date"] == submission_date:
                    ...
                    self.update_summary_stats(
                        number_requested=num_req,
                        number_saved=num_returned,
                        keyword="saved_request",
                    )
                    # save
                    with open(self.path_pipeline, "w") as file:
                        json.dump(self.photometry_pipeline, file, indent=4)
...
class PlotPhotometry:
    ...
    def plot_photometry_dates(self):
        ...
        plt.text(
            0.5, dateobs - 2 * 360, "Baseline", color="b", ha="center", va="bottom"
        )
