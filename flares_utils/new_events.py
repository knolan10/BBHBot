...
parser.add_argument(
    "--push", action="store_true", help="Enable push to public repository"
)
parser.add_argument("--verbose", action="store_false", help="Enable verbose output")
parser.add_argument(
    "--check_before_run",
    action="store_true",
    help="At certain points require user input to continue",
)
parser.add_argument(
    "--path_events_dictionary", default="data/", help="Path to the events dictionary"
)
parser.add_argument(
    "--mlp_modelpath", default="mlp_model.sav", help="Path to the MLP model"
)
...
df = NewEventsToDict(
    params,
    trigger_status,
    args.path_events_dictionary,
    check_before_run=args.check_before_run,
).save_data()
...
crossmatch = KowalskiCrossmatch(
    eventid, skymap_str, dateid, zmin, zmax, args.path_events_dictionary
)
...
df, priority, trigger_df, error_triggers = PushEventsPublic(
    args.path_events_dictionary, push=args.push, verbose=args.verbose
).format_and_push()
