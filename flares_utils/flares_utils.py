...
class FlarePreprocessing():
    ...
    def get_total_fluxes(self, df):
        ...
        Fluxunc_tot = np.sqrt(df2['forcediffimfluxunc'] **2 + nearestreffluxunc **2 ) #add variances (conservative estimate)
...
class RollingWindowStats():
    def __init__(self, graceid, agn, path_data, window_size_before=50, window_size_after=25, baseline_years=2, observing_run='O4c'):
        self.graceid = graceid
        self.agn = agn
        self.path_data = path_data
        ...
        # open the stored event info
        with open(f'{path_data}/flare_data/dicts/events_dict_{self.observing_run}.json', 'r') as file:
    ...
    def calculate_meds_mads(self, df):
        ...
        mads_post = []
        jds = []
        N_before = round(360 * self.baseline_years / self.window_size_before)  # number windows in baseline
        N_after = round(200 / self.window_size_after)  # number windows in 200 day post-GW period
...
class RollingWindowHeuristic:
    ...
    def medians_test(self):
        ...
        index_g = [i for i in range(len(stats_g)) 
                if not (np.isnan(stats_g[i][0]).all() or np.isnan(stats_g[i][1]).all() or np.isnan(stats_g[i][2]).all())
                and sum(x > np.nanmin(stats_g[i][2]) for x in np.array(stats_g[i][0]) - self.k_mad * np.array(stats_g[i][1])) / len(stats_g[0]) > self.percent]
        # r
        stats_r = [i[1] for i in self.rolling_stats]
        index_r = [i for i in range(len(stats_r)) 
                if not (np.isnan(stats_r[i][0]).all() or np.isnan(stats_r[i][1]).all() or np.isnan(stats_r[i][2]).all())
                and sum(x > np.nanmin(stats_r[i][2]) for x in np.array(stats_r[i][0]) - self.k_mad * np.array(stats_r[i][1])) / len(stats_r[0]) > self.percent]
        # i
    ...
    def check_photometry_coverage(self):  
        ...
        no_gw_points = [i for i in self.rolling_stats if is_all_nan(i[0][2]) and is_all_nan(i[1][2]) and is_all_nan(i[2][2])]
...
class Plotter:
    def __init__(self, index_to_plot, color_to_plot, agn, rolling_stats, graceid, path_data, observing_run='O4c', flares_from_graceid=False):
    ...
    def plot_all(self, index):
        ...
        for i, ax in enumerate(axes):  # i represents each filter color
            ...
            weighted_means = grouped.apply(lambda g: np.average(g['mag'], weights=g['sigma_mag']))
            mean_sigma_mag = grouped['sigma_mag'].mean()
            ax.plot(weighted_means.index, weighted_means, 'o', color=colors[i], markersize=3)
        ...
        if self.index_to_plot=='all':
            for i in range(0, len(self.agn)):
                try:
                    ...
                    print(e)
                    print(i)
        elif type(self.index_to_plot)==list:
